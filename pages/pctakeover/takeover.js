          ...
          netWork.httpPOST("/kcv1/pcUser/orderDetail", params, function(res) {
            if (res.code == 0) {
              var orderStatus = res.data.result.order_status;
               //  -3 超时取消 -2 司机取消 -1 用户取消  0 发单 1 已抢单 2 开始行程 3 发起收款 5 已付款
              if (orderStatus == 3 || orderStatus == 4) {
                wx.navigateTo({
                  url: "/pages/paythefare/paythefare?carType=pc"
                })
                clearTimeout(timer)
              }
              that.setData({
                ...
                score: res.data.result.driver_rank,
      ...
      success: function(res) {
        if (res.confirm) {
          netWork.paramsCB({
            ...
            netWork.httpPOST("/kcv1/pcUser/cancelOrder", params, function(res) {
              if (res.code == 0) {
                wx.showModal({
                  title: "取消订单成功",
                  success: function(res) {
